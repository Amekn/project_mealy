...
macos_targets=(macos-arm64-preview macos-x86_64-preview)
expected_assets=()
for target in "${linux_targets[@]}"; do
  case $target in
    linux-x86_64-gnu) deb="mealy_${debian_version}_amd64.deb" ;;
    linux-aarch64-gnu) deb="mealy_${debian_version}_arm64.deb" ;;
  esac
  expected_assets+=(
...
for target in "${macos_targets[@]}"; do
  ...
  if ! awk '
      ...
      END {if (NR != 2) exit 1}
    ' "$output_dir/$checksum"; then
    echo "macOS preview checksum manifest is not canonical: $checksum" >&2
    exit 65
  fi
  checksum_paths=$(awk '{print $2}' "$output_dir/$checksum" | sort)
  expected_paths=$(printf '%s\n' "$archive" "$sbom" | sort)
  if [[ $checksum_paths != "$expected_paths" ]]; then
    echo "macOS preview checksum inventory is not exact: $checksum" >&2
    exit 65
  fi
  if ! (cd "$output_dir" && sha256sum --check --strict "$checksum" >/dev/null); then
    echo "macOS preview checksum verification failed: $checksum" >&2
    exit 65
  fi
done

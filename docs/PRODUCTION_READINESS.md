...
|---|---|---|
| Runtime proof | Core durability and security contracts pass real process scenarios. | Passed for the current schema-16 runtime, including its clean exact-package 24-hour gate. |
...
history and resolved conversations, rejects force-pushes and deletion, and strictly requires the
six authoritative workspace, Linux sandbox/browser, Ubuntu x86-64/ARM64, and distribution
compatibility checks. Immutable releases and private vulnerability reporting are enabled. The
`live-provider-smoke` Environment is configured with an explicit required owner review and contains
...
the publication gate. A separate workflow-controlled CI event/SHA identity and selector require a
successful exact-commit `push` run on `main` at both package admission and publication; ancestry or
a green pull-request-only run is insufficient. The OpenRouter path discovers the
...
forces zero prices rather than accepting a secret-bearing arbitrary URL. The owner-local ChatGPT
subscription bridge is implemented through an exact-digest official Codex client; it defaults to
the current account-catalog recommendation during guided onboarding and applies a conservative
128,000-token Mealy context ceiling. The lower-level stopped-home command retains OpenAI's
maintained `gpt-5.6` alias as its automation default. The
bridge passed a live ChatGPT-sign-in activation probe and a complete governed-tool task
...
|---|---|---|---|
| Public-use license | Anyone receiving the public release has explicit terms permitting installation and use. | The copyright holder selected Apache-2.0 on 2026-07-15. The repository carries the canonical license text through the existing exact `license-file = "LICENSE"` inheritance, every workspace package inherits it, and the real-checkout plus fixture validators pass. | P0 acceptance is complete. |
| Packaging | Versioned Linux packages and archives install without a Rust toolchain; checksums, provenance/SBOM, uninstall, upgrade, and rollback are documented and tested. | A tag-gated matrix natively builds x86-64/ARM64 generic archives, Debian packages, and RPMs plus an x86-64 Arch package from one verified payload. Each format is passive: no maintainer script, service activation, install hook, credential access, or home mutation. Reproducibility tests compare independent package builds byte for byte; inventory, modes, ownership, checksums, version/schema identity, dependency contracts, and tamper rejection are fail closed. Clean pinned images build and exercise packages on Ubuntu 24.04/26.04, Debian 13, Fedora 44, and Arch Linux. The stable archive manager additionally provides locked active/previous slots, safe same-schema rollback, coordinated exact-digest cross-schema home activation, hard-kill compensation, and state-preserving uninstall. Every published asset receives checksums, a CycloneDX SBOM, dependency notices, and offline-verifiable GitHub provenance. Post-candidate source also builds signed APT, DNF, and Pacman repositories from those exact packages, attests their complete manifest, and requires clean public HTTPS installs on native x86-64/ARM64 runners. | Complete only for an exact tag whose native package, owner-key signing, Pages deployment, and dependent public distribution/repository jobs are green. The Pages site and production signing identity still require the documented one-time owner activation. |
| Onboarding and secrets | One guided command selects a provider/model, validates limits, stores only a secret reference in config, provisions an owner-private credential broker, and performs a bounded test call. | `mealyctl setup` now initializes a clean private home without booting the daemon, interactively selects `OpenAI`, Anthropic, `OpenRouter`, or literal-loopback local Responses, prompts only for non-secret model/limit/price values, imports a remote credential from a named environment variable, previews the exact non-secret settings/config digest, and requires typing `APPROVE` unless complete flags carry explicit non-interactive approval. The ordinary `mealyctl onboard` flow additionally uses one bounded echo-disabled terminal prompt when the route's named variable is absent, restores echo before continuing, and reuses the same zeroizing credential for discovery, probing, and broker activation; non-terminal absence fails before home mutation. It reuses the protocol-specific activation boundary: conflict-safe broker preflight, one no-tools up-to-256-output-token/30-second/1-MiB bounded JSON/SSE probe, atomic config/history publication, secret-free JSON, and exact daemon/doctor/chat handoff. Process tests cover clean-home remote probe/brokering, hidden-prompt OpenRouter/custom journeys with no terminal disclosure, interactive local setup, denial with zero provider state, staged-probe labeling, the official Codex subscription-client envelope with injected API-key variables excluded, maintained OpenAI subscription defaults, and Claude subscription rejection before mutation/invocation. ChatGPT subscription activation stores no session or secret reference: it pins the canonical official-client path/SHA-256 and fails closed on drift, invalid login, malformed output, missing usage, or over-limit output. Credentialless presets accept only literal loopback, send no authorization header, store `credential: null`, and create no broker entry. OpenRouter discovery filters account-visible text/tool models, normalizes limits/prices, and flags unsupported pricing axes; all discovery disables proxy/redirects and enforces bounded safe metadata. Failure bodies and credentials are never echoed or persisted in config. Exact fallback removal preserves the rest of the ordered chain and retains the old broker key; compatible primary replacement preserves the complete chain, while incompatible trust/locality/identity changes fail before mutation. Stopped-daemon revocation removes only unreferenced broker identities, and encrypted secret backup remains explicit. | Guided activation and controlled fallback rotation are complete. Direct OpenAI prices/remote limits still require operator verification, OpenRouter Responses remains upstream beta, and the ChatGPT bridge depends on the current official-client login and contract. |
| Interactive use | A first-party `chat` surface streams text/tool progress, renders approvals, supports interrupt/queue/steer, reconnects by cursor, and resumes existing sessions. | Bare terminal `mealyctl` selects onboarding for an unconfigured home and a new chat for a configured home; non-terminal use fails without mutation and names explicit commands. `mealyctl chat` creates/resumes sessions and keeps a bounded input reader plus per-admission watcher set active concurrently. `chat --continue`/`-c` selects the newest exact-binding session without creating a duplicate, `chat --pick` provides a terminal-only chooser over 20 recent exact-binding sessions, and `chat --session-id` selects an older session for automation; every resume path scans retained history to rediscover active/pending work. A pseudo-terminal process proof covers both bare-command journeys, exact non-latest selection, terminal enforcement, clean picker cancellation, latest-session selection, no duplicate creation, and a direct no-history recovery instruction. Plain/queue, steer, interrupt, exact-subject approve/deny, `/act`, `/edit`, `/manage`, and `/run` remain available during work; accepted work is correlated by durable causation/correlation IDs, promotion polling resumes by cursor after restart, and bounded non-authoritative provider text deltas, model/tool lifecycle progress, and terminal responses are rendered without confusing adjacent turns. `session send-file` additionally admits one owner-selected no-follow regular UTF-8 text/source file up to 256 KiB under digest-bound untrusted metadata without persisting its host path, reusing normal delivery/idempotency. The daemon projects a chronological, same-epoch suffix of at most 32 successful prior user/assistant turns under a 512-KiB discovery ceiling, reserves the latest authenticated input before optional history, honors the latest compaction cutoff, and drops prior-session-derived context on epoch rotation. A public wire test proves the exact second-turn projection and zero-dispatch replay; the workspace-revocation restart test proves old assistant text cannot reintroduce revoked authority. `session list` exposes the bounded JSON form of the exact-binding recent-session browser. Terminal control characters are neutralized. The pseudo-terminal suite also holds admission open, proves the next prompt remains interactive, then proves `/quit` exits without waiting; delayed-provider process exercise covers steer, interrupt, queue, cancellation, and correct attribution. | P0 acceptance is complete. A full-screen TUI, conversation-title previews, branching, and transcript export remain optional future UX rather than release requirements. |
| Model providers | OpenAI Responses plus at least one independently exercised local or alternate adapter support tools, classified retry/backoff, health, explicit fallback, cancellation, limits, and exact usage/cost settlement. | Separate `OpenAI` Responses and Anthropic Messages adapters implement their distinct authentication, request/tool, terminal, SSE, error, and usage contracts. Both are byte/text/event bounded, actively cancellable every 50 ms, independently health/accounted, and governed by persisted jittered retry plus conservative unknown-outcome rules. Responses terminal-match validation and Anthropic ordered block/cumulative-usage validation feed the same non-authoritative progress and canonical result boundary. A public-process proof makes a Responses primary fail definitely, dispatches the durable retry through a streaming Anthropic fallback, verifies both protocol/credential/request shapes and endpoint histories, and replays with zero network calls. The OpenRouter preset deliberately reuses this hardened stateless Responses boundary; process tests prove its exact bearer/store/stream/model probe plus account-filtered catalog limits/pricing normalization. The exact-digest official Codex adapter provides an owner-local ChatGPT subscription route without token extraction or API-key inheritance; onboarding uses bounded official account/login/model app-server methods with separate terminal consent and exact catalog selection, while runtime client tools/connectors are disabled, process channels and usage are bounded, structured decisions normalize to the same provider contract, and capability-recorded client input overhead is included in reservation, settlement, and replay. A process regression reports 8,000 client input tokens and proves the expanded reservation settles; the live OpenAI proof makes two model calls, executes one governed read tool, and completes with a citation and recorded-only replay. Legacy Claude subscription configuration fails closed before dispatch; Claude remains available through the direct Anthropic Messages API, strict-free OpenRouter, a custom endpoint, or Claude Code. Process tests additionally prove remote authenticated and literal-loopback credentialless request bounds, pagination/filter behavior, credential isolation, and failure redaction. A manual read-only-permission workflow is ready to run setup probe, durable turn, health/settlement/usage/replay/leak/drain against either a strict zero-price OpenRouter model or the fixed-origin authenticated private Responses endpoint. | P0 adapter, local preset, deterministic model discovery, and live OpenAI subscription tool proof are complete. A reviewed OpenRouter free-model workflow run remains an external release gate; owner-local Qwen3.6-27B llama-server acceptance passed setup, two model calls, one governed read, citation, exact zero-cost settlement, healthy accounting, recorded-only replay, credential-leak inspection, and clean drain; OpenRouter's upstream Responses contract is beta. |
...
selection, refuses silent home replacement, reuses the exact provider activation transaction,
installs/starts the Linux user service, and requires bounded health plus `doctor`. Process tests
cover strict free-OpenRouter catalog/probe behavior; official Codex account, browser/device login,
and account-catalog selection; terminal consent and non-mutation failures; Claude subscription
rejection; credential containment; clean-home configuration; and idempotent refusal. A live,
model-call-free acceptance against the installed Codex app-server selected the Plus account's
`gpt-5.6-sol` recommendation without exposing account or token material. This is next-release source
...
Bare terminal `mealyctl` now selects that onboarding transaction when configuration is absent and
opens a new durable chat when it is present. The selection is based only on a no-follow regular
configuration file; all three standard streams must be terminals, and automation must name an
explicit subcommand.
...
The packager rejects common user-home paths even if its caller bypasses
...
Ubuntu 24.04 package-owned systemd-mutation gates. The new daemon—not the earlier hash—was the next
...
RustSec finding. That tree also passed the full strict workspace suite, disposable
MinGW Windows and GCC AArch64 Linux all-target/all-feature checks, and a real Windows PE link after
normalizing Windows canonical paths without changing either Linux binary. Its planned 24-hour
clock was superseded before completion by the browser transport finding below.
...
`e96d0012fb07b62d033d385257e3cc3a1c75f93d3a256a8804e213405c2dcf90`; its subsequent 24-hour
attempt used that exact daemon but did not pass. Verifier-only commit `c797e8e` reproduced both binary hashes after closing
...
`ok`, complete replay, clean drain, and zero residual work. Both the pre-fix schema-15 run and the
later 832-turn web-policy run invalidated by the proxy-thread lifetime audit were intentionally
stopped rather than mislabeled. The final run selected a verified extracted release daemon
explicitly, bound that exact executable SHA-256 rather than merely identifying a Cargo integration
build, and retained its home on disk-backed Btrfs rather than the host's RAM-backed `tmpfs`.
...
successful and no residual work. The subsequent clean
[schema-15 release soak](benchmarks/2026-07-16-schema15-release-soak.json)
ran 86,425.217 seconds against corrected `mealyd` SHA-256
`4db797fd085ab845b7b30752a822168c670e6420df1edb22726c3e18eba64c97`, completed 15,824 turns,

...
|---|---|---|---|
| Public-use license | Anyone receiving the public release has explicit terms permitting installation and use. | The tag workflow now validates one reviewed workspace SPDX expression and matching project text, requires every package to inherit it, and accepts Apache-2.0, MIT, or dual `MIT OR Apache-2.0`; fixture tests reject restrictive or mismatched terms and metadata. | The copyright holder has not selected public terms. The current all-rights-reserved `LICENSE` expressly prohibits use, copying, modification, publication, and distribution, so a public-use release is legally blocked. |

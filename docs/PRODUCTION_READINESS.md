...
forces zero prices rather than accepting a secret-bearing arbitrary URL. Owner-local OpenAI
and Claude subscription bridges are implemented separately through exact-digest official clients;
the OpenAI bridge passed a live ChatGPT-sign-in activation probe and a complete governed-tool task
...
settled at zero cost, passed deterministic validation, and replayed with zero live calls. The
current Claude login fails closed as unavailable and must be refreshed before that optional route
can be claimed.
...
cover strict free-OpenRouter catalog/probe behavior, both subscription clients, credential

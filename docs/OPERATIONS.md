...
that matches the reviewed path, obtains the stopped-home lock, rechecks the generated bytes, removes
the unit, and reloads the user manager. The verifier recognizes an exact Mealy-generated unit even
when it points to a previous or explicitly selected daemon path. It always preserves the complete
Mealy home.

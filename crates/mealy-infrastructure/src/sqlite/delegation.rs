...
use mealy_application::{
    AcquireResourceClaimCommit, AgentLoopLimits, AgentStoreError, DelegationStore, DelegationView,
    OwnershipContext, PrepareDelegationCommit, RecordDelegationResultCommit, StartDelegationCommit,
    sha256_digest, validate_delegation_commit,
};
...
impl DelegationStore for SqliteStore {
    ...
    fn prepare_delegation(
        ...
        let parent_capabilities =
            serde_json::from_str::<CapabilityGrant>(&parent.capabilities_json)
        ...
        let validation_required =
            i64::from(commit.success_criteria.independent_validation_required());
        transaction
            .execute(
                "INSERT INTO task(\
    ...
    transaction
        .query_row(
            "SELECT run.task_id, run.capability_ceiling_json, run.correlation_id, \
                    lineage.root_run_id, lineage.depth \
             FROM run \
